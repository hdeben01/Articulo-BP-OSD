...
def main():
    ...
    for codeConfig in codesConfig:
        plt.plot(ps, PlsBPOSD_lib[codeConfig], marker="o", label=f"BPOSD_lib {codeConfig}")
        plt.plot(ps, PlsBPOSD[codeConfig], marker="s", label=f"BPOSD {codeConfig}")

    plt.yscale("log")   # eje Y logarítmico
    plt.xscale("linear") # el eje X lo dejamos lineal
    plt.xlabel("Physical error rate (p)")
    plt.ylabel("Logical error rate (P_L)")
    plt.title("Logical vs Physical Error Rates (BPOSD_lib vs BPOSD)")
    plt.grid(True, which="both", ls="--", lw=0.7)
    plt.legend()
    plt.tight_layout()

    # Guardar en PNG
    plt.savefig("logical_vs_physical_v1_72_10k", dpi=300)
